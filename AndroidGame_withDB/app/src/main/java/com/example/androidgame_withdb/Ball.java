 ...
 * Ball object.
 */
public class Ball extends com.example.androidgame_withdb.TexturedAlignedRect {
     ...
     */
    private static void computeCircleEdges(int[] left, int[] right) {
        ...
        while (x <= y) {
            setCircleValues(x, y, left, right);
     ...
     */
    private static void setCircleValues(int x, int y, int[] left, int[] right) {
        left[31 +y] = left[31 -y] = 31 - x;
        left[31 +x] = left[31 -x] = 31 - y;
        right[31 +y] = right[31 -y] = 31 + x;
        right[31 +x] = right[31 -x] = 31 + y;
    }
