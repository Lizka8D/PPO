...
public class BasicAlignedRect extends BaseRect {
    ...
    public static void createProgram() {
        sProgramHandle = com.example.androidgame.Util.createProgram(VERTEX_SHADER_CODE,
        ...
        com.example.androidgame.Util.checkGlError("glGetAttribLocation");

        // get handle to fragment shader's u_color member
        sColorHandle = GLES20.glGetUniformLocation(sProgramHandle, "u_color");
        com.example.androidgame.Util.checkGlError("glGetUniformLocation");
    ...
    public void setColor(float r, float g, float b) {
        com.example.androidgame.Util.checkGlError("setColor start");
        mColor[0] = r;
        mColor[1] = g;
        mColor[2] = b;
        mColor[3] = 1.0f;
    ...
    public static void prepareToDraw() {
        ...
        com.example.androidgame.Util.checkGlError("glUseProgram");

        // Enable the "a_position" vertex attribute.
        GLES20.glEnableVertexAttribArray(sPositionHandle);
        com.example.androidgame.Util.checkGlError("glEnableVertexAttribArray");
        ...
        com.example.androidgame.Util.checkGlError("glVertexAttribPointer");
     ...
     */
    public void draw() {
        if (GameSurfaceRenderer.EXTRA_CHECK) com.example.androidgame.Util.checkGlError("draw start");
        if (!sDrawPrepared) {
        ...
        if (GameSurfaceRenderer.EXTRA_CHECK) com.example.androidgame.Util.checkGlError("glUniformMatrix4fv");

        // Copy the color vector into the program.
        GLES20.glUniform4fv(sColorHandle, 1, mColor, 0);
        if (GameSurfaceRenderer.EXTRA_CHECK) com.example.androidgame.Util.checkGlError("glUniform4fv ");

        // Draw the rect.
        GLES20.glDrawArrays(GLES20.GL_TRIANGLE_STRIP, 0, VERTEX_COUNT);
        if (GameSurfaceRenderer.EXTRA_CHECK) com.example.androidgame.Util.checkGlError("glDrawArrays");
    }
}
